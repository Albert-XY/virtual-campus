...
export default function WeeklyReviewPage() {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const res = await fetch(`/api/reviews?period=weekly&date=${monday}`)
      if (res.ok) {
  ...
  const handleSubmit = async () => {
    if (mood === 0) {
      toast.error('请选择本周的心情')
      return
    }
    if (reviewText.trim().length < 10) {
      toast.error('本周回顾至少需要10个字')
      return
    }
    ...
    <div className="mx-auto max-w-lg px-4 py-6 space-y-5">
      ...
      {showCelebration && (
        <div className="celebration-overlay">
          <div className="celebration-content">
            ...
            <p className="celebration-text">周总结提交成功！</p>
      ...
      <Card>
        <CardContent className="pt-0 space-y-4">
          <h3 className="font-semibold" style={{ color: 'var(--accent-color)' }}>本周统计概览</h3>
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="p-3" style={{ backgroundColor: 'var(--bg-secondary)', borderRadius: 'var(--radius-sm)' }}>
              ...
              <p className="text-lg font-bold" style={{ color: 'var(--text-primary)' }}>
                {studyMinutes} <span className="text-xs font-normal" style={{ color: 'var(--text-muted)' }}>分钟</span>
              </p>
            </div>
            <div className="p-3" style={{ backgroundColor: 'var(--bg-secondary)', borderRadius: 'var(--radius-sm)' }}>
              <div className="flex items-center gap-1.5 mb-1">
          ...
          <Card>
            <CardContent className="pt-0 space-y-4">
              <div className="flex items-center justify-between">
                <h3 className="font-semibold" style={{ color: 'var(--accent-color)' }}>本周总结</h3>
              ...
              {review.tomorrow_plan && (
                <div className="p-3" style={{ backgroundColor: 'var(--accent-light)', borderRadius: 'var(--radius-sm)' }}>
                  <p className="text-xs font-medium mb-1" style={{ color: 'var(--accent-color)' }}>下周计划</p>
        ...
        <>
          ...
          <Card>
            <CardContent className="pt-0 space-y-3">
              <h3 className="font-semibold" style={{ color: 'var(--text-primary)' }}>这周心情如何？</h3>
          ...
          <Card>
            <CardContent className="pt-0 space-y-3">
              <h3 className="font-semibold" style={{ color: 'var(--text-primary)' }}>本周回顾</h3>
              <textarea
                value={reviewText}
                onChange={(e) => setReviewText(e.target.value)}
                placeholder="这周学得怎么样？目标完成了吗？"
          ...
          <Card>
            <CardContent className="pt-0 space-y-3">
              <h3 className="font-semibold" style={{ color: 'var(--text-primary)' }}>下周计划</h3>
              <textarea
                value={nextPlan}
                onChange={(e) => setNextPlan(e.target.value)}
                placeholder="下周打算重点做什么？"
                className="w-full resize-none p-3 text-sm outline-none transition-colors"
                style={{
                  minHeight: '100px',
                  borderRadius: 'var(--radius-sm)',
