...
// Component
// ============================================================
export default function MonthPlanTab() {
  const [loading, setLoading] = useState(true)
  const [existingPlan, setExistingPlan] = useState<MonthPlan | null>(null)
  const [lastMonthPlan, setLastMonthPlan] = useState<MonthPlan | null>(null)
  ...
  const fetchPlans = useCallback(async () => {
    ...
    try {
      ...
      const thisRes = await fetch('/api/monthly-plan')
      ...
      // Get last month's plan from history
      const lastMonthStart = getLastMonthStart()
      const historyRes = await fetch('/api/monthly-plan?action=history&limit=12')
      const historyData = historyRes.ok ? await historyRes.json() : null

      if (historyData?.plans) {
        const last = historyData.plans.find(
          (p: MonthPlan) => p.month_start === lastMonthStart
        )
        setLastMonthPlan(last ?? null)
      } else {
        setLastMonthPlan(null)
      }
    } catch (error) {
      console.error('获取月规划失败:', error)
    } finally {
      setLoading(false)
    }
  }, [])
  ...
  useEffect(() => {
    if (isEditing && existingPlan) {
      ...
      setSubjects(existingPlan.focus_areas ?? [])
  ...
  if (existingPlan && !isEditing) {
    return (
      <div className="space-y-4">
        <Card>
          <CardContent className="pt-0 space-y-4">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-xs" style={{ color: 'var(--text-muted)' }}>本月规划</p>
            ...
            {/* Goals */}
            <div>
              <p className="text-xs font-medium mb-2" style={{ color: 'var(--text-secondary)' }}>本月目标</p>
            ...
            {/* Subjects */}
            {existingPlan.focus_areas?.length > 0 && (
              <div>
                <p className="text-xs font-medium mb-2" style={{ color: 'var(--text-secondary)' }}>重点突破方向</p>
                <div className="flex flex-wrap gap-1.5">
                  {existingPlan.focus_areas.map((subject, i) => (
                    <Badge
                      key={i}
                      variant="secondary"
                      className="text-xs"
        ...
        {lastMonthPlan && (
          <div className="mt-6">
            <p className="text-xs font-medium mb-2" style={{ color: 'var(--text-muted)' }}>上月规划</p>
            <Card>
              <CardContent className="pt-0 space-y-3">
                <p className="text-xs" style={{ color: 'var(--text-muted)' }}>
                  {formatMonthLabel(lastMonthPlan.month_start)}
                </p>
                <ul className="space-y-1">
                  {lastMonthPlan.goals?.map((goal, i) => (
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="space-y-2">
        <h3 className="text-sm font-semibold" style={{ color: 'var(--accent-color)' }}>本月目标</h3>
        <p className="text-xs" style={{ color: 'var(--text-muted)' }}>设定本月要完成的学习目标</p>
      </div>
      ...
      {/* Subject tags */}
      <div className="space-y-2">
        <h3 className="text-sm font-semibold" style={{ color: 'var(--accent-color)' }}>重点突破方向</h3>
        <p className="text-xs" style={{ color: 'var(--text-muted)' }}>选择本月重点突破的科目</p>
      </div>
